...
it('should render up to 2 answers on page load', () => {
  const wrapper = mount(<AnswerList answers={dummyAnswers.results} />);
  const answerLen = wrapper.find('.answerList').filter(Answer).length;
...
it('should render all answers when clicking on see more answers button', () => {
  const wrapper = mount(<AnswerList answers={dummyAnswers.results} />);
  const oldAnswerLen = wrapper.find('.answerList').filter(Answer).length;
  ...
  const newAnswerLen = wrapper.find('.answerList').filter(Answer).length;
